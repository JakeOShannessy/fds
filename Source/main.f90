...
! Locate the back indices for WALL cells and THIN_WALL cells
...
IF (T>=MASS_CLOCK(MASS_COUNTER(1)) .AND. N_MPI_PROCESSES>1) THEN
   ...
   CALL MPI_GATHERV(REAL_BUFFER_MASS(0,DISP),COUNTS_MASS(MY_RANK),MPI_DOUBLE_PRECISION, &
                    MASS_DT(0,1),COUNTS_MASS,DISPLS_MASS,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ENDIF
...
IF (MY_RANK>0) THEN
   CALL MPI_GATHERV(REAL_BUFFER_QM(1,DISPLS(MY_RANK)+1),COUNTS_QM(MY_RANK),MPI_DOUBLE_PRECISION, &
                    REAL_BUFFER_QM(1,1),COUNTS_QM,DISPLS_QM,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ELSE
   CALL MPI_GATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,REAL_BUFFER_QM(1,1),COUNTS_QM,DISPLS_QM,&
...
IF (MY_RANK>0) THEN
   CALL MPI_GATHERV(REAL_BUFFER_HVAC(1,DISPLS(MY_RANK)+1),COUNTS_HVAC(MY_RANK),MPI_DOUBLE_PRECISION, &
                    REAL_BUFFER_HVAC(1,1),COUNTS_HVAC,DISPLS_HVAC,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ELSE
   CALL MPI_GATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,REAL_BUFFER_HVAC(1,1),COUNTS_HVAC,&
                    DISPLS_HVAC,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ENDIF
