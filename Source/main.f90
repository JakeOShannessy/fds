...
USE HVAC_ROUTINES
USE COMP_FUNCTIONS, ONLY : CURRENT_TIME,INITIALIZE_OUTPUT_CLOCKS
...
LOGICAL  :: EX=.FALSE.,DIAGNOSTICS,CTRL_STOP_STATUS,CHECK_FREEZE_VELOCITY=.TRUE.
...
CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,MAX_CELL_ASPECT_RATIO,COUNTS,DISPLS,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
...
SELECT CASE(TASK_NUMBER)
   ...
   CASE(1)
      ...
      ALLOCATE(REAL_BUFFER_QM(2*(N_Q_DOT+N_TRACKED_SPECIES),NMESHES))
      ...
      COUNTS_QM = COUNTS*2*(N_Q_DOT+N_TRACKED_SPECIES)
      DISPLS_QM = DISPLS*2*(N_Q_DOT+N_TRACKED_SPECIES)
      COUNTS_10 = COUNTS*10
      DISPLS_10 = DISPLS*10
      COUNTS_20 = COUNTS*20
      DISPLS_20 = DISPLS*20
...
MESH_LOOP_2A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_2B: DO NOM=1,NMESHES
      ...
      ALLOCATE(OS%LOGICALS(OS%N_LOGICALS_DIM))
      DO I=1,OS%N_ITEMS
         CALL ALLOCATE_STORAGE(NOM,WALL_INDEX=I+OS%START_INDEX-1,SURF_INDEX=OS%SURF_INDEX(I))
      ENDDO
   ENDDO MESH_LOOP_2B
ENDDO MESH_LOOP_2A
...
MESH_LOOP_4A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_4B: DO NOM=1,NMESHES
      ...
      ALLOCATE(OS%LOGICALS(OS%N_LOGICALS_DIM))
      DO I=1,OS%N_ITEMS
         CALL ALLOCATE_STORAGE(NOM,THIN_WALL_INDEX=I+OS%START_INDEX-1,SURF_INDEX=OS%SURF_INDEX(I))
      ENDDO
   ENDDO MESH_LOOP_4B
ENDDO MESH_LOOP_4A
...
IF (N_MPI_PROCESSES>1 .AND. N_ZONE>0) THEN
   ...
   ENDDO
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,INTEGER_BUFFER,N_ZONE,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,IERR)
   ...
   ENDDO
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,REAL_BUFFER,N_ZONE,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,INTEGER_BUFFER,N_ZONE,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,INTEGER_BUFFER_4,4*N_ZONE,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,IERR)
   DO IZ=1,N_ZONE
...
DO WHILE (ANY(SETUP_PRESSURE_ZONES_INDEX==0))
   ...
   IF (N_MPI_PROCESSES>1) THEN
      DO NM=1,NMESHES
         ...
         M => MESHES(NM)
         CALL MPI_BCAST(M%PRESSURE_ZONE,SIZE(M%PRESSURE_ZONE),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
      ENDDO
   ENDIF
   ...
   RECEIVING_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      IF (CODE==6 .AND. RNODE/=SNODE .AND. OS%N_ITEMS>0) THEN
         ...
         DO I=1,OS%N_ITEMS
            IW = OS%ITEM_INDEX(I)
            TW => M%THIN_WALL(IW)
            CALL PACK_THIN_WALL(NM,OS,TW,OS%SURF_INDEX(I),RC,IC,UNPACK_IT=.FALSE.,COUNT_ONLY=.FALSE.)
         ENDDO
      ENDIF
   ...
   SEND_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      RECEIVE_BACK_THIN_WALL: IF (CODE==6 .AND. SNODE/=RNODE .AND. OS%N_ITEMS>0) THEN
         ...
         DO I=1,OS%N_ITEMS
            ...
            CALL PACK_THIN_WALL(NOM,OS,TW,OS%SURF_INDEX(I),RC,IC,UNPACK_IT=.TRUE.,COUNT_ONLY=.FALSE.)
         ENDDO
...
IF (T>=HRR_CLOCK(HRR_COUNTER(1)) .AND. N_MPI_PROCESSES>1) CALL GATHER_Q_AND_M

! Gather species mass integrals to node 0

IF (T>=MASS_CLOCK(MASS_COUNTER(1)) .AND. N_MPI_PROCESSES>1) THEN
...
REAL_BUFFER_QM(1:N_Q_DOT,1:NMESHES)                                                   = Q_DOT(1:N_Q_DOT,1:NMESHES)
REAL_BUFFER_QM(N_Q_DOT+1:2*N_Q_DOT,1:NMESHES)                                         = Q_DOT_SUM(1:N_Q_DOT,1:NMESHES)
REAL_BUFFER_QM(2*N_Q_DOT+1:2*N_Q_DOT+N_TRACKED_SPECIES,1:NMESHES)                     = M_DOT(1:N_TRACKED_SPECIES,1:NMESHES)
REAL_BUFFER_QM(2*N_Q_DOT+N_TRACKED_SPECIES+1:2*N_Q_DOT+2*N_TRACKED_SPECIES,1:NMESHES) = M_DOT_SUM(1:N_TRACKED_SPECIES,1:NMESHES)
IF (MY_RANK>0) THEN
...
ENDIF
IF (MY_RANK==0) THEN
   Q_DOT(1:N_Q_DOT,1:NMESHES)               = REAL_BUFFER_QM(1:N_Q_DOT,1:NMESHES)
   Q_DOT_SUM(1:N_Q_DOT,1:NMESHES)           = REAL_BUFFER_QM(N_Q_DOT+1:2*N_Q_DOT,1:NMESHES)
   M_DOT(1:N_TRACKED_SPECIES,1:NMESHES)     = REAL_BUFFER_QM(2*N_Q_DOT+1:2*N_Q_DOT+N_TRACKED_SPECIES,1:NMESHES)
   M_DOT_SUM(1:N_TRACKED_SPECIES,1:NMESHES) = REAL_BUFFER_QM(2*N_Q_DOT+N_TRACKED_SPECIES+1:2*N_Q_DOT+2*N_TRACKED_SPECIES,1:NMESHES)
ENDIF
