...
REAL(EB) :: MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            ...
            POLYNOMIAL_TEMP(4),POLYNOMIAL_COEFF(9,3)
...
CHARACTER(LABEL_LENGTH) :: RAMP_CP,RAMP_CP_L,RAMP_K,RAMP_MU,RAMP_D,RADCAL_ID,RAMP_G_F,SPEC_ID(MAX_SPECIES),POLYNOMIAL
...
NAMELIST /REAC/ A, A_TROE, A_LOW_PR, AIT_EXCLUSION_ZONE,AIT_EXCLUSION_ZONE_CTRL_ID,AIT_EXCLUSION_ZONE_DEVC_ID, &
      ...
      IF (RN%FUEL=='null') THEN
         FIND_FUEL: DO NS=1,RN%N_SMIX
            IF (RN%NU_READ(NS)<-TWO_EPSILON_EB) THEN
               ...
               DO NS2=1,N_TRACKED_SPECIES
                  IF (RN%NU(NS2) /= 0._EB .AND. TRIM(RN%SPEC_ID_NU_READ(NS))==TRIM(SPECIES_MIXTURE(NS2)%ID)) THEN
                     RN%FUEL_SMIX_INDEX = NS2
                     EXIT
                  ENDIF
               ENDDO
               IF(RN%FUEL_SMIX_INDEX /= -1) EXIT FIND_FUEL
            ENDIF
         ENDDO FIND_FUEL
      ELSE
...
MINIMUM_LAYER_THICKNESS = -1.E-5_EB  ! The absolute value is the default, m
MLRPUA                  = 0._EB
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (.NOT.ALLOCATED(SF%N_LAYER_CELLS)) THEN
      ...
      SF%N_CELLS_INI  = 1
      SF%N_MATL   = 1
      ALLOCATE(SF%MATL_NAME(SF%N_MATL)) ; SF%MATL_NAME = 'null'
